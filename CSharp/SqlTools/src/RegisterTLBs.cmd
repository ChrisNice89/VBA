...
SET BaseDir=%0
SET BaseDir=%BaseDir:"=%
SET BaseDir=%BaseDir:\RegisterTLBs.cmd=%
...
del %BinDir%\%TargetName%.dll 
move %BinDir%\%SourceName%.dll %BinDir%\%TargetName%.dll 
cls


ECHO Registering %TargetName%.dll


SET BinPath=%BinDir%\%TargetName%.dll 

%NETPATH%\regasm.exe "%BinPath%" /codebase  /tlb:"%TargetDir%\%TargetName%.tlb" /nologo /silent
